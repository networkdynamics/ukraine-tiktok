...
def main():
    sample_type = 'all_related'

    this_dir_path = os.path.dirname(os.path.abspath(__file__))
    video_sample_path = os.path.join(this_dir_path, '..', '..', 'data', 'outputs', f'{sample_type}_apr_video_sample.csv')
    desc_video_sample_path = os.path.join(this_dir_path, '..', '..', 'data', 'outputs', f'desc_{sample_type}_apr_video_sample.csv')
    ...
    with PyTok(chrome_version=111) as api:
        for id, video in video_sample.iterrows():
            if 'my_desc' in video and not pd.isna(video['my_desc']):
                continue
            try:
